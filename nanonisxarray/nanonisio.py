...
class Spectrum(NanonisFile):
    ...
    def _build_xarray(self, savecdf=None, avg_only=True):
        ...
        # data_dict = self.signals

        # # Extract the first key-pair for coordinates
        # coord_key = list(data_dict.keys())[0]
       
        # coords = {coord_key: data_dict.pop(coord_key)}
        # print(len(coords))
        # #del data_dict[coord_key]
        # # Convert the rest of the dictionary to an xarray DataSet
        # ds = xr.Dataset(self.signals, coords=coords)

        # print(ds)

        # print('.............')

        # sweep_name = [k for k in self.signals.keys() if "ias" in k];

        
        sweep_key = list(self.signals.keys())[0]
        if 'bias' in sweep_key.lower():
            sweep_name = 'bias'
        else:
        ...
        ds = xr.Dataset(coords={sweep_name:self.signals[sweep_key]})
        ...
        for k, v in ldict.items():
            ...
            for m in ([i for i in _signals if k in i]):
                ...
                ds[sname] = ((sweep_name), self.signals[m])
